...
from models import (
    ...
    ConsumoItemContratinho, ConsumoItemEmpenho, ItemContrato, Aditivo, CotaUnidadeItem,
    Log, Comentario, Processo, Fornecedor
)
# --- FORMULÁRIOS ---
from forms import (
    ...
    FornecedorForm, RelatorioPorFornecedorForm, ProcessoForm
)
import reports

# >>>>> ADIÇÃO 1: Importe o blueprint do novo arquivo <<<<<
...
def format_br_currency_filter(value):
    ...
    try:
        ...
        return locale.currency(num_value, symbol=True, grouping=True)
...
def format_quantity_filter(value):
    ...
    try:
        num_value = float(value)
        if num_value == int(num_value): return str(int(num_value))
...
def admin_listar_usuarios():
    ...
    elif sort_by == 'unidade_saude':
        query = query.join(UnidadeSaude, isouter=True)
        query = query.order_by(UnidadeSaude.nome_unidade.asc() if order == 'asc' else UnidadeSaude.nome_unidade.desc())
    ...
    if form.validate_on_submit():
        try:
            new_user = User(
                ...
                role=form.role.data,
                unidade_saude=form.unidade_saude_id.data
            )
...
def admin_editar_usuario(user_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            user.unidade_saude = form.unidade_saude_id.data
        ...
        form.unidade_saude_id.data = user.unidade_saude
...
def dashboard():
    ...
    if unidade_id_filtro == 'todas':
        itens_saldo_baixo = ItemAta.query.filter(
            ...
            ((ItemAta.saldo_disponivel * 1.0) / ItemAta.quantidade_registrada) <= percentual_limite
    ...
    query_empenhos = Empenho.query.filter(Empenho.data_fim_vigencia != None, Empenho.data_fim_vigencia <= data_alerta_prazo_obj)

    if filtro_vencidos == 'ocultar':
        hoje_date = hoje_obj.date()
        query_atas = query_atas.filter(Ata.data_validade >= hoje_date)
        query_contratos = query_contratos.filter(Contrato.data_fim_vigencia >= hoje_date)
        query_contratinhos = query_contratinhos.filter(Contratinho.data_fim_vigencia >= hoje_date)
        query_empenhos = query_empenhos.filter(Empenho.data_fim_vigencia >= hoje_date)
    ...
    empenhos_vencendo = query_empenhos.order_by(Empenho.data_fim_vigencia).all()
    ...
    return render_template('dashboard.html',
                           ...
                           empenhos_vencendo=empenhos_vencendo,
                           dias_alerta_prazo_atual=dias_prazo_atual,
                           hoje_datetime=hoje_obj,
                           filtro_vencidos_ativo=filtro_vencidos,
                           recent_comments=recent_comments)

# --- ROTAS CRUD PARA PROCESSOS ---
@app.route('/processos')
@login_required
def listar_processos():
    sort_by = request.args.get('sort_by', 'ano')
    order = request.args.get('order', 'desc')

    query = Processo.query

    if sort_by == 'numero_processo':
        query = query.order_by(Processo.numero_processo.asc() if order == 'asc' else Processo.numero_processo.desc())
    elif sort_by == 'ano':
        query = query.order_by(Processo.ano.asc() if order == 'asc' else Processo.ano.desc())
    elif sort_by == 'descricao':
        query = query.order_by(Processo.descricao.asc() if order == 'asc' else Processo.descricao.desc())
    else:
        query = query.order_by(Processo.ano.desc(), Processo.numero_processo.desc())
...
def criar_processo():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            registrar_log(current_user, "CRIOU PROCESSO", f"Processo '{novo_processo.numero_processo}/{novo_processo.ano}' (ID: {novo_processo.id}) criado.")
            flash('Processo cadastrado com sucesso!', 'success')
            return redirect(url_for('listar_processos'))
        except Exception as e:
            db.session.rollback()
            flash(f'Ocorreu um erro inesperado ao criar o processo: {e}', 'danger')

    return render_template('form_processo.html', form=form, titulo_pagina="Cadastrar Novo Processo")
...
def editar_processo(processo_id):
    ...
    form.id.data = processo.id

    if form.validate_on_submit():
        try:
            processo.numero_processo = form.numero_processo.data
            processo.ano = form.ano.data
            processo.descricao = form.descricao.data
            db.session.commit()
            registrar_log(current_user, "EDITOU PROCESSO", f"Processo '{processo.numero_processo}/{processo.ano}' (ID: {processo_id}) atualizado.")
            flash('Processo atualizado com sucesso!', 'success')
            return redirect(url_for('listar_processos'))
        except Exception as e:
            db.session.rollback()
            flash(f'Ocorreu um erro inesperado ao editar o processo: {e}', 'danger')

    return render_template('form_processo.html', form=form, titulo_pagina="Editar Processo")
...
@app.route('/processo/excluir/<int:processo_id>', methods=['POST'])
@login_required
@role_required('admin')
def excluir_processo(processo_id):
    processo = Processo.query.get_or_404(processo_id)

    if processo.atas.first() or processo.contratos.first() or processo.contratinhos.first() or processo.empenhos.first():
        flash('Não é possível excluir este processo, pois ele está vinculado a um ou mais documentos (Atas, Contratos, etc.).', 'danger')
        return redirect(url_for('listar_processos'))

    try:
        numero_processo = f"{processo.numero_processo}/{processo.ano}"
        db.session.delete(processo)
        db.session.commit()
        registrar_log(current_user, "EXCLUIU PROCESSO", f"Processo '{numero_processo}' (ID: {processo_id}) foi excluído.")
...
def listar_fornecedores():
    sort_by = request.args.get('sort_by', 'nome_fantasia')
    order = request.args.get('order', 'asc')

    query = Fornecedor.query

    if sort_by == 'nome_fantasia':
        query = query.order_by(Fornecedor.nome_fantasia.asc() if order == 'asc' else Fornecedor.nome_fantasia.desc())
    elif sort_by == 'razao_social':
        query = query.order_by(Fornecedor.razao_social.asc() if order == 'asc' else Fornecedor.razao_social.desc())
    elif sort_by == 'cnpj':
        query = query.order_by(Fornecedor.cnpj.asc() if order == 'asc' else Fornecedor.cnpj.desc())
    else:
    ...
    return render_template('listar_fornecedores.html',
                           titulo_pagina="Fornecedores Cadastrados",
...
def criar_fornecedor():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            registrar_log(current_user, "CRIOU FORNECEDOR", f"Fornecedor '{novo_fornecedor.nome_fantasia}' (ID: {novo_fornecedor.id}) criado.")
            flash('Fornecedor cadastrado com sucesso!', 'success')
            return redirect(url_for('listar_fornecedores'))
        except IntegrityError:
            db.session.rollback()
            flash('Erro: Já existe um fornecedor com este CNPJ.', 'danger')
        except Exception as e:
            db.session.rollback()
            flash(f'Ocorreu um erro inesperado: {e}', 'danger')

    return render_template('form_fornecedor.html', form=form, titulo_pagina="Cadastrar Novo Fornecedor", action="create")
...
def editar_fornecedor(fornecedor_id):
    ...
    form.id.data = fornecedor.id # Popula o ID para a validação
    if form.validate_on_submit():
        try:
            form.populate_obj(fornecedor)
            db.session.commit()
            registrar_log(current_user, "EDITOU FORNECEDOR", f"Fornecedor '{fornecedor.nome_fantasia}' (ID: {fornecedor_id}) atualizado.")
    ...
    return render_template('form_fornecedor.html', form=form, titulo_pagina="Editar Fornecedor", action="edit")

@app.route('/fornecedor/visualizar/<int:fornecedor_id>')
@login_required
def visualizar_fornecedor(fornecedor_id):
    fornecedor = Fornecedor.query.get_or_404(fornecedor_id)
    comment_form = CommentForm()
    return render_template('visualizar_fornecedor.html',
                           titulo_pagina=f"Detalhes de {fornecedor.nome_fantasia}",
...
def excluir_fornecedor(fornecedor_id):
    ...
    if fornecedor.atas or fornecedor.contratos or fornecedor.contratinhos or fornecedor.empenhos:
        flash('Não é possível excluir este fornecedor, pois ele está vinculado a um ou mais documentos (Atas, Contratos, etc.).', 'danger')
        return redirect(url_for('listar_fornecedores'))
    ...
    try:
        ...
        db.session.commit()
        registrar_log(current_user, "EXCLUIU FORNECEDOR", f"Fornecedor '{nome_fornecedor}' (ID: {fornecedor_id}) foi excluído.")
...
def listar_atas():
    ...
    if sort_by == 'numero_ata':
        query_base = query_base.order_by(Ata.numero_ata.asc()) if order == 'asc' else query_base.order_by(Ata.numero_ata.desc())
    elif sort_by == 'ano':
        query_base = query_base.order_by(Ata.ano.asc()) if order == 'asc' else query_base.order_by(Ata.ano.desc())
    elif sort_by == 'descricao':
        query_base = query_base.order_by(Ata.descricao.asc()) if order == 'asc' else query_base.order_by(Ata.descricao.desc())
    elif sort_by == 'data_validade':
        query_base = query_base.order_by(Ata.data_validade.asc()) if order == 'asc' else query_base.order_by(Ata.data_validade.desc())
    else: # Default
        query_base = query_base.order_by(Ata.ano.desc(), Ata.numero_ata.desc())

    todas_as_atas = query_base.all()
        ...
        if file and file.filename.endswith('.csv'):
            try:
                ...
                for row in csv_reader:
                    ...
                    if fornecedor_cnpj:
                        cnpj_limpo = ''.join(filter(str.isdigit, fornecedor_cnpj))

                        fornecedor = Fornecedor.query.filter_by(cnpj=cnpj_limpo).first()
                        if not fornecedor:
                            fornecedor = Fornecedor(
                                cnpj=cnpj_limpo,
                                nome_fantasia=row.get('fornecedor_nome', f'Fornecedor CNPJ {cnpj_limpo}'),
                                razao_social=row.get('fornecedor_razao_social')
                            )
                            db.session.add(fornecedor)
                            db.session.flush()
                        ata_obj.fornecedor_rel = fornecedor
                ...
                return redirect(url_for('listar_atas'))
    ...
    if form.validate_on_submit():
        try:
            nova_ata = Ata(
                ...
                fornecedor_rel=form.fornecedor_rel.data,
                processo=form.processo.data
            )
            db.session.add(nova_ata)
            db.session.commit()
            registrar_log(current_user, "CRIOU ATA", f"Ata {nova_ata.numero_ata}/{nova_ata.ano} (ID: {nova_ata.id}) criada.")
            flash('Ata criada com sucesso! Adicione itens a ela se necessário.', 'success')
            return redirect(url_for('listar_atas'))
        except IntegrityError:
            db.session.rollback()
            flash('Erro: Já existe uma ata com este número.', 'danger')
        except Exception as e:
            db.session.rollback()
            flash(f'Ocorreu um erro: {e}', 'danger')
    return render_template('criar_ata.html', form=form, titulo_pagina="Criar Nova Ata")
...
def editar_ata(ata_id):
    ...
    if form.validate_on_submit():
        try:
            form.populate_obj(ata)
            ata.processo = form.processo.data
            db.session.commit()
            registrar_log(current_user, "EDITOU ATA", f"Ata {ata.numero_ata}/{ata.ano} (ID: {ata_id}) atualizada.")
            ...
            db.session.rollback()
            flash(f'Ocorreu um erro: {e}', 'danger')
    form.processo.data = ata.processo
    return render_template('editar_ata.html', form=form, titulo_pagina="Editar Ata", ata_id=ata_id)
...
def listar_itens_da_ata(ata_id):
    ...
    sort_by = request.args.get('sort_by', 'descricao_item')
    order = request.args.get('order', 'asc')

    query_itens = ItemAta.query.filter_by(ata_id=ata.id)

    if sort_by == 'descricao_item':
        query_itens = query_itens.order_by(ItemAta.descricao_item.asc()) if order == 'asc' else query_itens.order_by(ItemAta.descricao_item.desc())
    elif sort_by == 'tipo_item':
        query_itens = query_itens.order_by(ItemAta.tipo_item.asc()) if order == 'asc' else query_itens.order_by(ItemAta.tipo_item.desc())
    elif sort_by == 'unidade_medida':
        query_itens = query_itens.order_by(ItemAta.unidade_medida.asc()) if order == 'asc' else query_itens.order_by(ItemAta.unidade_medida.desc())
    elif sort_by == 'quantidade_registrada':
        query_itens = query_itens.order_by(ItemAta.quantidade_registrada.asc()) if order == 'asc' else query_itens.order_by(ItemAta.quantidade_registrada.desc())
    elif sort_by == 'saldo_disponivel':
        query_itens = query_itens.order_by(ItemAta.saldo_disponivel.asc()) if order == 'asc' else query_itens.order_by(ItemAta.saldo_disponivel.desc())
    elif sort_by == 'valor_unitario_registrado':
        query_itens = query_itens.order_by(ItemAta.valor_unitario_registrado.asc()) if order == 'asc' else query_itens.order_by(ItemAta.valor_unitario_registrado.desc())
    elif sort_by == 'lote':
        query_itens = query_itens.order_by(ItemAta.lote.asc()) if order == 'asc' else query_itens.order_by(ItemAta.lote.desc())
    else: # Default
        query_itens = query_itens.order_by(ItemAta.descricao_item.asc())

    itens = query_itens.all()

    return render_template('listar_itens_da_ata.html',
...
def listar_contratos():
    filtro = request.args.get('filtro', 'vigentes')
    sort_by = request.args.get('sort_by', 'data_fim_vigencia')
    order = request.args.get('order', 'asc')
    ...
    if sort_by == 'numero_contrato':
        query_base = query_base.order_by(Contrato.numero_contrato.asc()) if order == 'asc' else query_base.order_by(Contrato.numero_contrato.desc())
    elif sort_by == 'objeto':
        query_base = query_base.order_by(Contrato.objeto.asc()) if order == 'asc' else query_base.order_by(Contrato.objeto.desc())
    elif sort_by == 'fornecedor':
        query_base = query_base.join(Fornecedor).order_by(Fornecedor.nome_fantasia.asc() if order == 'asc' else Fornecedor.nome_fantasia.desc())
    elif sort_by == 'data_fim_vigencia':
        query_base = query_base.order_by(Contrato.data_fim_vigencia.asc()) if order == 'asc' else query_base.order_by(Contrato.data_fim_vigencia.desc())
    else: # Default
        query_base = query_base.order_by(Contrato.data_assinatura_contrato.desc(), Contrato.numero_contrato.desc())

    todos_os_contratos = query_base.all()
    ...
    if form.validate_on_submit():
        try:
            novo_contrato_db = Contrato(
                ...
                data_fim_vigencia_original=form.data_fim_vigencia_original.data,
                data_fim_vigencia=form.data_fim_vigencia_original.data,
                unidade_saude_responsavel=form.unidade_saude_responsavel.data,
                processo=form.processo.data
            )
...
def editar_contrato(contrato_id):
    ...
    if request.method == 'POST':
        ...
        form.processo.data = contrato_para_editar.processo
    ...
    if form.validate_on_submit():
        try:
            ...
            contrato_para_editar.fornecedor_rel = form.fornecedor_rel.data
            ...
            contrato_para_editar.data_fim_vigencia_original = form.data_fim_vigencia_original.data
            contrato_para_editar.data_fim_vigencia = form.data_fim_vigencia_original.data # Recalculado por aditivos
            contrato_para_editar.unidade_saude_responsavel=form.unidade_saude_responsavel.data
            contrato_para_editar.processo = form.processo.data
...
def _recalculate_contract_state(contrato_id):
    ...
    valor_base = sum(item.valor_total_item for item in contrato.itens_do_contrato if item.valor_total_item is not None)
...
@login_required
def listar_unidades():
    sort_by = request.args.get('sort_by', 'nome_unidade')
    order = request.args.get('order', 'asc')

    query_base = UnidadeSaude.query

    if sort_by == 'nome_unidade':
        query_base = query_base.order_by(UnidadeSaude.nome_unidade.asc()) if order == 'asc' else query_base.order_by(UnidadeSaude.nome_unidade.desc())
    elif sort_by == 'tipo_unidade':
        query_base = query_base.order_by(UnidadeSaude.tipo_unidade.asc()) if order == 'asc' else query_base.order_by(UnidadeSaude.tipo_unidade.desc())
    elif sort_by == 'endereco':
        query_base = query_base.order_by(UnidadeSaude.endereco.asc()) if order == 'asc' else query_base.order_by(UnidadeSaude.endereco.desc())
    elif sort_by == 'telefone':
        query_base = query_base.order_by(UnidadeSaude.telefone.asc()) if order == 'asc' else query_base.order_by(UnidadeSaude.telefone.desc())
    elif sort_by == 'email_responsavel':
        query_base = query_base.order_by(UnidadeSaude.email_responsavel.asc()) if order == 'asc' else query_base.order_by(UnidadeSaude.email_responsavel.desc())
    else: # Default
        query_base = query_base.order_by(UnidadeSaude.nome_unidade.asc())

    unidades = query_base.all()
...
def _helper_popula_choices_itens_subform(form_principal, subform_fieldlist, ata_id_principal):
    item_choices = [('', '--- Selecione uma Ata Principal Primeiro ---')]
    if ata_id_principal is not None:
        try:
            id_ata_int = int(ata_id_principal)
            itens_da_ata_selecionada = ItemAta.query.filter_by(ata_id=id_ata_int).filter(ItemAta.saldo_disponivel > 0).order_by(ItemAta.descricao_item).all()
...
def listar_contratinhos():
    filtro = request.args.get('filtro', 'vigentes')
    sort_by = request.args.get('sort_by', 'data_emissao')
    order = request.args.get('order', 'desc')
    ...
    if sort_by == 'numero_contratinho':
        query_base = query_base.order_by(Contratinho.numero_contratinho.asc()) if order == 'asc' else query_base.order_by(Contratinho.numero_contratinho.desc())
    elif sort_by == 'objeto':
        query_base = query_base.order_by(Contratinho.objeto.asc()) if order == 'asc' else query_base.order_by(Contratinho.objeto.desc())
    elif sort_by == 'favorecido':
        query_base = query_base.join(Fornecedor).order_by(Fornecedor.nome_fantasia.asc() if order == 'asc' else Fornecedor.nome_fantasia.desc())
    elif sort_by == 'data_emissao':
        query_base = query_base.order_by(Contratinho.data_emissao.asc()) if order == 'asc' else query_base.order_by(Contratinho.data_emissao.desc())
    elif sort_by == 'data_fim_vigencia':
        query_base = query_base.order_by(Contratinho.data_fim_vigencia.asc()) if order == 'asc' else query_base.order_by(Contratinho.data_fim_vigencia.desc())
    elif sort_by == 'ata_vinculada':
        query_base = query_base.join(Ata).order_by(Ata.numero_ata.asc()) if order == 'asc' else query_base.join(Ata).order_by(Ata.numero_ata.desc())
    elif sort_by == 'unidade_saude':
        query_base = query_base.join(UnidadeSaude, isouter=True).order_by(UnidadeSaude.nome_unidade.asc()) if order == 'asc' else query_base.join(UnidadeSaude, isouter=True).order_by(UnidadeSaude.nome_unidade.desc())
    else: # Default
        query_base = query_base.order_by(Contratinho.data_emissao.desc())

    todos_os_contratinhos = query_base.all()
...
def criar_contratinho():
    ...
    id_ata_para_choices = form.ata_id.data.id if form.ata_id.data is not None else None
    ...
    if form.validate_on_submit():
        try:
            novo_contratinho = Contratinho(
                ...
                ata_vinculada=form.ata_id.data,
                unidade_saude_solicitante=form.unidade_saude_id.data,
            ...
            _helper_popula_choices_itens_subform(form, form.itens_consumidos, form.ata_id.data.id if form.ata_id.data else None)
...
def editar_contratinho(contratinho_id):
    ...
    if request.method == 'POST':
        ...
        form.processo.data = ct_para_editar.processo
    ...
    ata_id_atual_ou_selecionada = form.ata_id.data.id if hasattr(form.ata_id.data, 'id') else (form.ata_id.data if form.ata_id.data else ct_para_editar.ata_id)
    ...
    if form.validate_on_submit():
        try:
            ...
            ct_para_editar.fornecedor_rel = form.fornecedor_rel.data
            ct_para_editar.data_emissao = form.data_emissao.data
            ct_para_editar.data_fim_vigencia = form.data_fim_vigencia.data
            ct_para_editar.ata_vinculada = form.ata_id.data
            ct_para_editar.unidade_saude_solicitante = form.unidade_saude_id.data
            ct_para_editar.valor_total_manual = form.valor_total_manual.data
            ct_para_editar.processo = form.processo.data
...
def listar_empenhos():
    ...
    query_base = Empenho.query
    hoje = datetime.now(timezone.utc).date()

    if filtro == 'vigentes':
        query_base = query_base.filter(or_(Empenho.data_fim_vigencia >= hoje, Empenho.data_fim_vigencia == None))

    if sort_by == 'numero_empenho':
        query_base = query_base.order_by(Empenho.numero_empenho.asc()) if order == 'asc' else query_base.order_by(Empenho.numero_empenho.desc())
    elif sort_by == 'descricao_simples':
        query_base = query_base.order_by(Empenho.descricao_simples.asc()) if order == 'asc' else query_base.order_by(Empenho.descricao_simples.desc())
    elif sort_by == 'favorecido':
        query_base = query_base.join(Fornecedor).order_by(Fornecedor.nome_fantasia.asc() if order == 'asc' else Fornecedor.nome_fantasia.desc())
    elif sort_by == 'data_emissao':
        query_base = query_base.order_by(Empenho.data_emissao.asc()) if order == 'asc' else query_base.order_by(Empenho.data_emissao.desc())
    elif sort_by == 'data_fim_vigencia':
        query_base = query_base.order_by(Empenho.data_fim_vigencia.asc()) if order == 'asc' else query_base.order_by(Empenho.data_fim_vigencia.desc())
    elif sort_by == 'ata_vinculada':
        query_base = query_base.join(Ata).order_by(Ata.numero_ata.asc()) if order == 'asc' else query_base.join(Ata).order_by(Ata.numero_ata.desc())
    elif sort_by == 'unidade_saude':
        query_base = query_base.join(UnidadeSaude, isouter=True).order_by(UnidadeSaude.nome_unidade.asc()) if order == 'asc' else query_base.join(UnidadeSaude, isouter=True).order_by(UnidadeSaude.nome_unidade.desc())
    else: # Default
        query_base = query_base.order_by(Empenho.data_emissao.desc())

    todos_os_empenhos = query_base.all()
...
def editar_empenho(empenho_id):
    ...
    if request.method == 'POST':
        ...
        form.processo.data = emp_para_editar.processo
        while len(form.itens_consumidos.entries) > 0: form.itens_consumidos.pop_entry()
    ...
    ata_id_atual_ou_selecionada = form.ata_id.data.id if hasattr(form.ata_id.data, 'id') else (form.ata_id.data if form.ata_id.data else emp_para_editar.ata_id)
    _helper_popula_choices_itens_subform(form, form.itens_consumidos, ata_id_atual_ou_selecionada)
    ...
    if form.validate_on_submit():
        try:
            ...
            emp_para_editar.fornecedor_rel = form.fornecedor_rel.data
            emp_para_editar.data_emissao = form.data_emissao.data
            emp_para_editar.data_fim_vigencia = form.data_fim_vigencia.data
            emp_para_editar.ata_vinculada = form.ata_id.data
            emp_para_editar.unidade_saude_solicitante = form.unidade_saude_id.data
            emp_para_editar.valor_total_manual = form.valor_total_manual.data
            emp_para_editar.processo = form.processo.data

            id_ata_para_processar_itens = form.ata_id.data.id if form.ata_id.data else emp_para_editar.ata_id
            _, novo_valor_total_dos_itens = _processar_itens_consumidos_para_salvar(
...
def relatorio_consumo_unidade():
    ...
    if form.validate_on_submit():
        unidade_id = form.unidade_saude_id.data.id
        ...
        ).select_from(ConsumoItemContratinho).join(Contratinho).join(UnidadeSaude).join(ItemAta).join(Ata)\
        ...
        ).select_from(ConsumoItemEmpenho).join(Empenho).join(UnidadeSaude).join(ItemAta).join(Ata)\
...
def relatorio_consumo_item():
    ...
    if form.validate_on_submit():
        item_ata_id = form.item_ata_id.data.id
        data_inicio = form.data_inicio.data
        data_fim = form.data_fim.data
...
def relatorio_contratos_vigentes_unidade():
    ...
    if form.validate_on_submit():
        ...
        contratos_vigentes = Contrato.query.filter(
            Contrato.unidade_saude_responsavel.has(id=unidade_id),
        ...
        empenhos_vinculados = Empenho.query.filter(
            Empenho.unidade_saude_id == unidade_id,
            or_(Empenho.data_fim_vigencia >= hoje, Empenho.data_fim_vigencia == None)
    ...
    return render_template('selecionar_relatorio_contratos_vigentes_unidade.html', form=form, titulo_pagina="Relatório de Documentos Vigentes por Unidade")
...
@app.route('/relatorios/por_fornecedor', methods=['GET', 'POST'])
@login_required
@role_required('admin', 'gestor', 'leitura')
def relatorio_por_fornecedor():
    form = RelatorioPorFornecedorForm()
    if form.validate_on_submit():
        fornecedor = form.fornecedor.data
        if fornecedor:
            return reports.gerar_pdf_relatorio_fornecedor(fornecedor)
        else:
            flash('Fornecedor não encontrado.', 'danger')

    return render_template('selecionar_relatorio_fornecedor.html',
                           form=form,
                           titulo_pagina="Relatório por Fornecedor")

# >>>>> INÍCIO DO NOVO CÓDIGO DE PESQUISA <<<<<
@app.route('/search')
@login_required
def search():
    ...
    if not query_str:
        flash('Por favor, digite um termo para pesquisar.', 'info')
        # Tenta voltar para a página anterior, ou para o dashboard como padrão
        return redirect(request.referrer or url_for('dashboard'))

    search_pattern = f"%{query_str}%"

    # Pesquisa em Processos
    processos_encontrados = Processo.query.filter(
        or_(
            Processo.numero_processo.ilike(search_pattern),
            Processo.descricao.ilike(search_pattern),
            cast(Processo.ano, db.String).ilike(search_pattern)
        )
    ).limit(15).all()
    for processo in processos_encontrados:
        results.append({
            'tipo': 'Processo',
            'titulo': f'Processo {processo.numero_processo}/{processo.ano}',
            'descricao': processo.descricao or "Sem descrição.",
            'url': url_for('visualizar_processo', processo_id=processo.id)
        })

    # Pesquisa em Fornecedores
    fornecedores_encontrados = Fornecedor.query.filter(
        or_(
            Fornecedor.nome_fantasia.ilike(search_pattern),
            Fornecedor.razao_social.ilike(search_pattern),
            Fornecedor.cnpj.ilike(search_pattern)
        )
    ).limit(15).all()
    for fornecedor in fornecedores_encontrados:
        results.append({
            ...
            'descricao': f"CNPJ: {fornecedor.cnpj}",
            'url': url_for('visualizar_fornecedor', fornecedor_id=fornecedor.id)
        })

    # Pesquisa em Atas
    atas_encontradas = Ata.query.filter(
        or_(
            Ata.numero_ata.ilike(search_pattern),
            Ata.descricao.ilike(search_pattern),
            cast(Ata.ano, db.String).ilike(search_pattern)
        )
    ).limit(15).all()
    for ata in atas_encontradas:
        results.append({
            'tipo': 'Ata de Registro',
            'titulo': f'Ata {ata.numero_ata}/{ata.ano}',
            'descricao': ata.descricao or "Sem descrição.",
            'url': url_for('listar_itens_da_ata', ata_id=ata.id)
        })

    # Pesquisa em Itens de Ata
    itens_ata_encontrados = ItemAta.query.filter(
        or_(
            ItemAta.descricao_item.ilike(search_pattern),
            ItemAta.lote.ilike(search_pattern)
        )
    ).limit(15).all()
    for item in itens_ata_encontrados:
        results.append({
            'tipo': 'Item de Ata',
            'titulo': item.descricao_item,
            'descricao': f"Pertence à Ata {item.ata_mae.numero_ata}/{item.ata_mae.ano}",
            'url': url_for('listar_itens_da_ata', ata_id=item.ata_id)
        })

    # Pesquisa em Contratos
    contratos_encontrados = Contrato.query.filter(
        or_(
            Contrato.numero_contrato.ilike(search_pattern),
            Contrato.objeto.ilike(search_pattern)
        )
    ).limit(15).all()
    for contrato in contratos_encontrados:
        results.append({
            'tipo': 'Contrato',
            'titulo': f"Contrato {contrato.numero_contrato}",
            'descricao': contrato.objeto,
            'url': url_for('visualizar_contrato', contrato_id=contrato.id)
        })

    # Pesquisa em Contratinhos
    contratinhos_encontrados = Contratinho.query.filter(
        or_(
            Contratinho.numero_contratinho.ilike(search_pattern),
            Contratinho.objeto.ilike(search_pattern)
        )
    ).limit(15).all()
    for contratinho in contratinhos_encontrados:
        results.append({
            'tipo': 'Contratinho',
            'titulo': f"Contratinho {contratinho.numero_contratinho}",
            'descricao': contratinho.objeto or "Sem objeto.",
            'url': url_for('visualizar_contratinho', contratinho_id=contratinho.id)
        })

    # Pesquisa em Empenhos
    empenhos_encontrados = Empenho.query.filter(
        or_(
            Empenho.numero_empenho.ilike(search_pattern),
            Empenho.descricao_simples.ilike(search_pattern)
        )
    ).limit(15).all()
    for empenho in empenhos_encontrados:
        results.append({
            ...
            'descricao': empenho.descricao_simples or "Sem descrição.",
            'url': url_for('visualizar_empenho', empenho_id=empenho.id)
        })
    ...
    return render_template('search_results.html',
                           ...
                           titulo_pagina=f"Resultados da Busca por '{query_str}'")

# >>>>> FIM DO NOVO CÓDIGO DE PESQUISA <<<<<

# >>>>> ADIÇÃO 2: Registre o blueprint na aplicação (adicione esta linha antes do if __name__ == '__main__') <<<<<
...
# Fim do arquivo completo: app.py