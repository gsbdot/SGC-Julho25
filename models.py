...
# === NOVOS MODELOS E MODELOS PRINCIPAIS ATUALIZADOS ===

class Fornecedor(db.Model, Commentable):
    ...
    # Back-references 'atas', 'contratos', 'contratinhos', 'empenhos'
    # são criadas pelos relacionamentos nos modelos correspondentes.

    def __repr__(self):
        return f'<Fornecedor {self.nome_fantasia}>'

# --- MODELO NOVO ---
class Processo(db.Model, Commentable):
    ...
    # Relacionamentos (o backref cria o atributo 'processo' nos modelos de documento)
    atas = db.relationship('Ata', backref='processo', lazy='dynamic')
    contratos = db.relationship('Contrato', backref='processo', lazy='dynamic')
    contratinhos = db.relationship('Contratinho', backref='processo', lazy='dynamic')
    empenhos = db.relationship('Empenho', backref='processo', lazy='dynamic')
    
    # Garante que a combinação de número e ano do processo seja única
    __table_args__ = (UniqueConstraint('numero_processo', 'ano', name='_numero_ano_processo_uc'),)
...
class Ata(db.Model, Commentable):
    __tablename__ = 'ata'
    id = db.Column(db.Integer, primary_key=True)
    ...
    fornecedor_id = db.Column(db.Integer, db.ForeignKey('fornecedor.id', name='fk_ata_fornecedor_id'), nullable=True)
    fornecedor_rel = db.relationship('Fornecedor', backref='atas')
...
class Contrato(db.Model, Commentable):
    ...
    fornecedor_id = db.Column(db.Integer, db.ForeignKey('fornecedor.id', name='fk_contrato_fornecedor_id'), nullable=True)
    fornecedor_rel = db.relationship('Fornecedor', backref='contratos')
...
class Contratinho(db.Model, Commentable):
    ...
    fornecedor_id = db.Column(db.Integer, db.ForeignKey('fornecedor.id', name='fk_contratinho_fornecedor_id'), nullable=True)
    fornecedor_rel = db.relationship('Fornecedor', backref='contratinhos')
...
class Empenho(db.Model, Commentable):
    ...
    fornecedor_id = db.Column(db.Integer, db.ForeignKey('fornecedor.id', name='fk_empenho_fornecedor_id'), nullable=True)
    fornecedor_rel = db.relationship('Fornecedor', backref='empenhos')
...
# === DEMAIS MODELOS (SEM ALTERAÇÃO DIRETA NESTA ETAPA) ===
...
class Aditivo(db.Model, Commentable):
