...
# Importa o novo modelo Fornecedor
from models import ItemAta, Contrato, Contratinho, Empenho, Fornecedor
...
def gerar_pdf_lista_atas(todas_as_atas):
    ...
    table_data = [
        [Paragraph("Nº Ata/Ano", styles['TableHeader']),
         Paragraph("Fornecedor", styles['TableHeader']),
    ...
    for ata in todas_as_atas:
        table_data.append([
            ...
            Paragraph(ata.fornecedor_rel.nome_fantasia if ata.fornecedor_rel else '-', styles['TableCell']),
            Paragraph(ata.data_validade.strftime('%d/%m/%Y') if ata.data_validade else '-', styles['TableCell'])
        ])
    
    ata_table = Table(table_data, colWidths=[1.5*inch, 4.0*inch, 1.7*inch], repeatRows=1)
...
def gerar_pdf_detalhes_ata(ata, itens_da_ata):
    ...
    # Campo de Fornecedor atualizado
    fornecedor_nome = ata.fornecedor_rel.nome_fantasia if ata.fornecedor_rel else 'Não informado'
    story.append(Paragraph(f"<b>Fornecedor:</b> {fornecedor_nome}", styles['InfoGeral']))
...
def gerar_pdf_detalhes_contrato(contrato):
    ...
    # Campo de Fornecedor atualizado
    fornecedor_nome = contrato.fornecedor_rel.nome_fantasia if contrato.fornecedor_rel else 'Não informado'
    story.append(Paragraph(f"<b>Fornecedor:</b> {fornecedor_nome}", styles['InfoGeral']))
...
def gerar_pdf_detalhes_contratinho(contratinho):
    ...
    # Campo de Favorecido atualizado
    favorecido_nome = contratinho.fornecedor_rel.nome_fantasia if contratinho.fornecedor_rel else 'Não informado'
    story.append(Paragraph(f"<b>Favorecido:</b> {favorecido_nome}", styles['InfoGeral']))
...
def gerar_pdf_detalhes_empenho(empenho):
    ...
    # Campo de Favorecido atualizado
    favorecido_nome = empenho.fornecedor_rel.nome_fantasia if empenho.fornecedor_rel else 'Não informado'
    story.append(Paragraph(f"<b>Favorecido:</b> {favorecido_nome}", styles['InfoGeral']))
    
    story.append(Paragraph(f"<b>Data de Emissão:</b> {empenho.data_emissao.strftime('%d/%m/%Y') if empenho.data_emissao else '-'}", styles['InfoGeral']))
    story.append(Paragraph(f"<b>Data Fim de Vigência:</b> {empenho.data_fim_vigencia.strftime('%d/%m/%Y') if empenho.data_fim_vigencia else '-'}", styles['InfoGeral']))
...
def gerar_pdf_contratos_vigentes_unidade(unidade, contratos, contratinhos, empenhos):
    ...
    if contratos:
        ...
        for c in contratos:
            data.append([
                ...
                Paragraph(c.fornecedor_rel.nome_fantasia if c.fornecedor_rel else '-', styles['TableCell']),
    ...
    if empenhos:
        ...
        data = [[Paragraph(h, styles['TableHeader']) for h in ["Nº Empenho", "Descrição", "Ata Vinculada", "Data de Emissão", "Fim da Vigência"]]]
        for e in empenhos:
            data.append([
                ...
                Paragraph(e.data_fim_vigencia.strftime('%d/%m/%Y') if e.data_fim_vigencia else '-', styles['TableCell']),
            ])
        t = Table(data, colWidths=[1.5*inch, 4*inch, 1.5*inch, 1.5*inch, 1.5*inch], repeatRows=1)
        t.setStyle(table_style)
        story.append(t)
...
# --- NOVA FUNÇÃO DE RELATÓRIO POR FORNECEDOR ---
def gerar_pdf_relatorio_fornecedor(fornecedor):
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4,
                            rightMargin=0.75*inch, leftMargin=0.75*inch,
                            topMargin=1*inch, bottomMargin=1*inch)
    ...
    story.append(Paragraph(f"Relatório do Fornecedor", styles['TituloRelatorio']))
    story.append(Paragraph(f"<b>{fornecedor.nome_fantasia}</b>", styles['Subtitulo']))
    story.append(Paragraph(f"Data de Emissão: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}", styles['InfoData']))

    # Dados Cadastrais
    story.append(Paragraph(f"<b>CNPJ:</b> {fornecedor.cnpj}", styles['InfoGeral']))
    if fornecedor.razao_social:
        story.append(Paragraph(f"<b>Razão Social:</b> {fornecedor.razao_social}", styles['InfoGeral']))
    if fornecedor.endereco:
        story.append(Paragraph(f"<b>Endereço:</b> {fornecedor.endereco}", styles['InfoGeral']))
    if fornecedor.telefone:
        story.append(Paragraph(f"<b>Telefone:</b> {fornecedor.telefone}", styles['InfoGeral']))
    if fornecedor.email:
        story.append(Paragraph(f"<b>Email:</b> {fornecedor.email}", styles['InfoGeral']))
    ...
    # Atas Vinculadas
    if fornecedor.atas:
        story.append(Paragraph("Atas de Registro de Preço Vinculadas", styles['Subtitulo']))
        data = [[Paragraph(h, styles['TableHeader']) for h in ["Nº/Ano", "Descrição", "Validade"]]]
        for ata in fornecedor.atas:
            data.append([
                Paragraph(f"{ata.numero_ata}/{ata.ano}", styles['TableCell']),
                Paragraph(ata.descricao or '-', styles['TableCell']),
                Paragraph(ata.data_validade.strftime('%d/%m/%Y') if ata.data_validade else '-', styles['TableCell']),
            ])
        t = Table(data, colWidths=[1.2*inch, 4.3*inch, 1.5*inch], repeatRows=1)
        t.setStyle(table_style)
        story.append(t)
        story.append(Spacer(1, 0.2*inch))
    ...
    if fornecedor.contratos:
        story.append(Paragraph("Contratos Vinculados", styles['Subtitulo']))
        data = [[Paragraph(h, styles['TableHeader']) for h in ["Nº Contrato", "Objeto", "Vigência", "Valor"]]]
        for contrato in fornecedor.contratos:
            data.append([
                Paragraph(contrato.numero_contrato, styles['TableCell']),
                Paragraph(contrato.objeto[:80] + '...' if len(contrato.objeto) > 80 else contrato.objeto, styles['TableCell']),
                Paragraph(contrato.data_fim_vigencia.strftime('%d/%m/%Y') if contrato.data_fim_vigencia else '-', styles['TableCell']),
                Paragraph(_format_br_currency_pdf(contrato.valor_global_contrato), styles['TableCellRight']),
            ])
        t = Table(data, colWidths=[1.2*inch, 3.5*inch, 1.1*inch, 1.2*inch], repeatRows=1)
        t.setStyle(table_style)
        story.append(t)
        story.append(Spacer(1, 0.2*inch))
        
    # Documentos de Consumo (Contratinhos e Empenhos)
    docs_consumo = list(fornecedor.contratinhos) + list(fornecedor.empenhos)
    docs_consumo.sort(key=lambda x: x.data_emissao, reverse=True)
    
    if docs_consumo:
        story.append(Paragraph("Contratinhos e Empenhos Vinculados", styles['Subtitulo']))
        data = [[Paragraph(h, styles['TableHeader']) for h in ["Tipo", "Número", "Data Emissão", "Valor Total"]]]
        # >>>>> CORREÇÃO APLICADA AQUI <<<<<
        for doc_consumo in docs_consumo:
            tipo = "Contratinho" if isinstance(doc_consumo, Contratinho) else "Empenho"
            numero = doc_consumo.numero_contratinho if isinstance(doc_consumo, Contratinho) else doc_consumo.numero_empenho
            valor = doc_consumo.valor_total_manual if doc_consumo.valor_total_manual is not None else doc_consumo.valor_total_itens
            data.append([
                Paragraph(tipo, styles['TableCell']),
                Paragraph(numero, styles['TableCell']),
                Paragraph(doc_consumo.data_emissao.strftime('%d/%m/%Y'), styles['TableCell']),
                Paragraph(_format_br_currency_pdf(valor), styles['TableCellRight']),
            ])
        t = Table(data, colWidths=[1.2*inch, 1.8*inch, 1.5*inch, 2.5*inch], repeatRows=1)
        t.setStyle(table_style)
        story.append(t)

    # Comentários
    if fornecedor.comments.count() > 0:
        story.append(Paragraph("Comentários sobre o Fornecedor", styles['Subtitulo']))
        for comment in fornecedor.comments:
            story.append(Paragraph(f"{comment.author.username} em {comment.timestamp.strftime('%d/%m/%Y %H:%M')}:", styles['CommentUser']))
            story.append(Paragraph(comment.content.replace('\n', '<br/>'), styles['CommentContent']))
            story.append(Spacer(1, 0.1*inch))
    ...
    response = make_response(pdf_value)
    response.headers['Content-Type'] = 'application/pdf'
    nome_arquivo = f"relatorio_fornecedor_{fornecedor.nome_fantasia.replace(' ', '_').lower()}.pdf"
    response.headers['Content-Disposition'] = f'inline; filename={nome_arquivo}'
    return response
